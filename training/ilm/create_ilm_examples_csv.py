...
if __name__ == '__main__':
  ...
  parser.add_argument('--text_column', type=str, default='text', help='Name of column containing text')
  parser.add_argument('--seed', type=int)
  parser.add_argument('--sample_size', type=int, help='Number of random samples to use (if not specified, uses all)')
  ...
  # Load data from CSV
  print('Loading CSV file: {}'.format(args.csv_file))
  df = pd.read_csv(args.csv_file)

  # Extract text column
  texts = df[args.text_column].dropna().tolist()
  print('Loaded {} documents from CSV'.format(len(texts)))

  # Apply sampling if specified
  if args.sample_size is not None:
    if args.sample_size > len(texts):
      print('Warning: sample_size ({}) is larger than dataset size ({}). Using full dataset.'.format(
          args.sample_size, len(texts)))
    else:
      texts = random.sample(texts, args.sample_size)
      print('Sampled {} documents'.format(len(texts)))

  # Apply max_num_documents limit if specified
  if args.max_num_documents is not None:
    texts = texts[:args.max_num_documents]
  ...
  print('Processed {} documents and created {} examples per document (expected {})'.format(
    num_documents,
    num_masked_examples / num_documents if num_documents > 0 else 0,
    args.num_examples_per_document))
  ...
  print('Saved results to {}/{}.pkl'.format(args.out_dir, args.tag))
